...
| `classPrefix`              | `string`                                    | 'carousel'                       | Prefix for every CSS class the carousel renders (`carousel`, `carousel__slide`, ...). Change it when the default collides with other CSS or scripts. The bundled stylesheet targets the default prefix; see [Styling](/components/carousel#styling). <Badge text="0.19.0"/> |
