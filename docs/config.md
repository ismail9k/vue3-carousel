...
| `autoScrollOnFocus`        | `boolean`                                   | true                             | When true, focusing a slide (or an element inside it) via keyboard navigates to that slide. Set to false to leave focus in place, e.g. for slides whose links open another page. <Badge text="0.19.0"/> |
