...
| `autoScrollOnFocus`        | `boolean`                                   | true                             | When true, focusing a slide or an element inside it by keyboard or programmatically (not by mouse or touch) slides the carousel to that slide. Set to false to keep the carousel in place, for example when slide links open another page. Content inside off-screen slides is never in the tab order; with false, focusing it programmatically no longer scrolls it into view. <Badge text="0.19.0"/> |
